<template lang="">
    <div>
        <h1> Edit news </h1>
        <form @submit.prevent="updateNews(nouvelle)">
            ...
            <div class="form-group">
                ...
                <input v-model="nouvelle.video" type="text" id="image" class="form-control">
            </div>
            <div class="form-group">
...
export default {
    ...
    methods: {
    async  updateNews(nouvelle){
            //  if(this.nouvelle.title == "" || this.nouvelle.content == '' || this.nouvelle.date == ""){
            //      return;
            //  }
            const res = await db.collection("news").doc(this.id).update(nouvelle)
